...
{   echo $OUTPUT->notification("Your need to specify 'userid' for single GDPR deletion or 'limit' fo number of deletions!");
	...
	User by ID: <input name="userid" type="text" size="8" style="<?php echo $buttonStyle;?>" value="<?php echo $userid;?>"><br><b>OR</b><br>
	Maximum number of deletions <input name="limit" type="text" size="5" style="<?php echo $buttonStyle;?>" value="<?php echo $limit;?>"> 
	with <input name="lastaccess" type="text" size="3" style="<?php echo $buttonStyle;?>" value="<?php echo $lastaccess;?>"> Days since last access<br>
...
$ufilter = $limitN = "";
if ($userid )
{ 	$ufilter = "id = $userid"; $limitN = "LIMIT 1"; }
else 
...
print "\n<hr><b>Found $hits ASH user accounts " .($dryrun ?"ready for GDPR deletion (DryRun)" :"to be GDPR deleted") . "!</b><br>\nQuery: $sql<br><br>\n"; 
if ( $userid AND !$hits )
...
{ 	$cnt++; //if ( $cnt>=80) { break;}
	$msg = $cnt . ". $user->username ($user->id) - $user->firstname $user->lastname - $user->email - Deleted: $user->deleted - Lastaccess: ".
			gmdate("Y-m-d", $user->lastaccess) ." - created: ". gmdate("Y-m-d", $user->timecreated)."<br>\n";
	print $msg;
	if ( $user) {
		if ($user->id == $USER->id) 
		{ // Self deletion attempt.
			echo $OUTPUT->notification("You tried to delete your own user account. This is not permitted!"); continue; }
		elseif ( is_siteadmin($user) ) // can't delete a siteadmin account
		{	echo $OUTPUT->notification("You tried to delete an administrator account. This is not permitted!"); continue; } 
	}
	...
	if ( !$dryrun )
	{	set_time_limit(180);
		if ( $user->deleted ) { $DB->execute("UPDATE {user} SET deleted=0,suspended=0 WHERE id=$user->id"); }
		if ( !($user = \core_user::get_user($user->id) ) )
		{ 	$errCnt++; $errMsg .= $cnt . ". " . $msg . ob_get_clean()."\n";	
			continue;
		}
